...
async function getDirectoryHandle(uri: URI): Promise<FileSystemDirectoryHandle | undefined> {
	if (isRoot(uri)) {
		return HTMLFileSystemDirectoryHandlers.get(uri.authority);
	}
	...
	const parent = await getDirectoryHandle(URI.from({ ...uri, path: splitResult[0] }));
	return await parent?.getDirectoryHandle(extUri.basename(uri));
}

async function getParentDirectoryHandle(uri: URI): Promise<FileSystemDirectoryHandle | undefined> {
	return getDirectoryHandle(URI.from({ ...uri, path: extUri.dirname(uri).path }));
}

async function getFileHandle(uri: URI): Promise<FileSystemFileHandle | undefined> {
	const result = HTMLFileSystemFileHandlers.get(uri.authority);

	if (result) {
		return result;
	}

	const parent = await getParentDirectoryHandle(uri);
...
export class HTMLFileSystemProvider implements IFileSystemProviderWithFileReadWriteCapability {
	...
	async readFile(resource: URI): Promise<Uint8Array> {
		const handle = await getFileHandle(resource);
	...
	async stat(resource: URI): Promise<IStat> {
		if (HTMLFileSystemFileHandlers.has(resource.authority)) {
			return {
				type: FileType.File,
				mtime: 0,
				ctime: 0,
		...
		const parent = await getParentDirectoryHandle(resource);
	...
	async readdir(resource: URI): Promise<[string, FileType][]> {
		const parent = await getDirectoryHandle(resource);
